...
/**
 * Market-data host. Unlike trading, market data uses a single host for both
 * paper and live accounts, so the `paper` flag has no effect here (accepted for
 * symmetry with the trading `Configuration`).
 */
...
// Re-exported from the shared error module so `marketData.ApiError` (etc.)
// resolve to the SAME class identity as `trading.*` and the top-level `errors.*`.
export {
    ResponseError,
    ApiError,
    AuthError,
...
export interface ConfigurationParameters {
    ...
    paper?: boolean; // accepted for symmetry with trading; market data uses one host, so this has no effect
