...
export interface AlpacaWebSocketOptions {
    ...
    reconnect?: boolean;
    /** Grow the reconnect delay on repeated failures. Default true. */
    backoff?: boolean;
    /** Seconds added to the reconnect delay each attempt. Default 0.5. */
    backoffIncrement?: number;
    /** Cap (seconds) for a single reconnect delay. Default 30. */
    maxReconnectSec?: number;
...
export abstract class AlpacaWebSocket extends EventEmitter {
    ...
    private readonly backoffIncrement: number;
    ...
    constructor(options: AlpacaWebSocketOptions) {
        ...
        this.backoffIncrement = options.backoffIncrement ?? 0.5;
        this.maxReconnectSec = options.maxReconnectSec ?? 30;
    ...
    private scheduleReconnect(): void {
        ...
        const seconds = this.backoff
            ? Math.min(this.reconnectSeconds, this.maxReconnectSec)
            : Math.max(this.reconnectSeconds, 1);
        this.log(`reconnecting in ${seconds}s`);
        this.reconnectTimer = setTimeout(() => {
            if (this.backoff) {
                this.reconnectSeconds = Math.min(
                    this.reconnectSeconds + this.backoffIncrement,
